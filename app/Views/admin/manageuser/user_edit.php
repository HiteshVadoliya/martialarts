            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    ...
                    <div class="card card-primary card-outline">
                        ...
                        <!-- form start -->    
                        <form class="form-horizontal" name="form_add" id="form_add" method="post" action="<?php echo $url.'admin/manageuser/user_edit/'.$id; ?>" enctype="multipart/form-data" accept-charset="utf-8">
                            <div class="card-body">
                                <?php /* ?>
                                            ...
                                            <?php
                                                if(count($user_role_arr) > 0){
                                                    foreach($user_role_arr as $val){
                                                        if($val['user_role_id'] == $user_rec['user_role_pid']){
                                                            echo '<option value="'.$val['user_role_id'].'" selected>'.$val['role'].'</option>';
                                                        }
                                                        else{
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="fv_fname" name="fv_fname" value="<?=$user_rec['fname'];?>">
                                </div>

                                <div class="form-group">
                                    <label for="fv_lname">Last Name</label>
                                    <input type="text" class="form-control" id="fv_lname" name="fv_lname" value="<?=$user_rec['lname'];?>">
                                </div>

                                <div class="form-group">
                                    <label for="fv_lname">Username</label>
                                    <input type="text" class="form-control" id="fv_username" name="fv_username" value="<?=$user_rec['username'];?>">
                                </div>

                                <?php /* ?>
                                <div class="form-group">
                                    <label for="fv_password">Password</label>
                                    <input type="text" class="form-control" id="fv_password" name="fv_password" value="<?=$user_rec['password'];?>">
                                </div>
                                <?php /**/ ?>

                                <div class="form-group">
                                    <label for="fv_email">Email address</label>
                                    <input type="email" class="form-control" id="fv_email" name="fv_email" value="<?=$user_rec['email'];?>" />
                                </div>

                                <div class="form-group">
                                    <label for="fv_phone">Phone Number</label>
                                    <input type="tel" class="form-control" id="fv_phone" name="fv_phone" value="<?=$user_rec['phone'];?>" />
                                </div>

                                <div class="form-group">
                                    <label for="fv_phone">Phone System ID</label>
                                    <input type="text" class="form-control" id="fv_phonesys_id" name="fv_phonesys_id"  value="<?=$user_rec['phonesys_id'];?>" />
                                </div>

                                <div class="form-group">
                                    <label for="fv_is_active">User Is Active</label>
                                    <select class="form-control select" id="fv_is_active" name="fv_is_active">
                                        <option value="">Please select Status</option>
                                        <option value="1" <?php if($user_rec['is_active'] == '1'){ echo 'selected'; } ?> >Yes</option>
                                        <option value="0" <?php if($user_rec['is_active'] == '0'){ echo 'selected'; } ?>>No</option>
                                    </select>
                                </div>
